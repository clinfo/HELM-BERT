#!/usr/bin/env python3
"""Simple dataset deduplication for Propedia v2 using canonical SMILES."""
...
# Input files (check both data/ and local_data/)
INPUT_FILES = {
...
log_dir = Path("outputs") / "preprocessing" / f"propedia_v2_deduplication_{timestamp}"
...
def deduplicate_dataset(df: pd.DataFrame, dataset_name: str) -> pd.DataFrame:
    ...
    df = df.sort_values(by=smiles_col, na_position='last').reset_index(drop=True)
...
def main():
    """Main deduplication process for Propedia v2."""
    logger.info("Starting Propedia v2 deduplication with full cross-dataset deduplication...")

#!/usr/bin/env python3
"""
Process Propedia v2.3 data to extract ALL positive PPI samples to CSV format.
...
# Add scripts/ to path so helpers can be imported
sys.path.insert(0, str(Path(__file__).resolve().parent))

from helpers.paths import PROPEDIA_RAW_DIR, STAGE_DIRS

from datetime import datetime

# Create output directory for logs
timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
log_dir = Path("outputs") / "preprocessing" / f"propedia_v2_processing_{timestamp}"
...
class PropediaV2Processor:
    """Main processor for Propedia v2 data."""
    ...
    def __init__(self, base_path: str):
        ...
        Args:
            base_path: Base path to propedia_v2 directory
        """
...
def main():
    ...
    base_path = str(PROPEDIA_RAW_DIR)
    output_file = str(STAGE_DIRS["ingested"] / "propedia.csv")
    
    logger.info("Starting Propedia v2 positive sample extraction (NO FILTERING)...")
    ...
    try:
        ...
        processor = PropediaV2Processor(base_path)

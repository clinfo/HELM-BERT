#!/usr/bin/env python
"""Visualize multi-assay permeability train/val/test split distributions via t-SNE.

Uses Morgan fingerprints (ECFP4) from SMILES to embed molecules into 2D.
Mirrors the format of visualize_permeability_splits.py.

Usage:
    # Combined figure: Random + Scaffold side by side (default)
    python scripts/visualize_multi_assay_splits.py

    # Single split only
    python scripts/visualize_multi_assay_splits.py --split random
    python scripts/visualize_multi_assay_splits.py --split scaffold

    # Legend bar only
    python scripts/visualize_multi_assay_splits.py --legend

Output:
    results/visualization/tsne_multi_assay_splits_{timestamp}.pdf/.png
    results/visualization/tsne_multi_assay_{split}_{timestamp}.pdf/.png
    results/visualization/tsne_multi_assay_legend_{timestamp}.pdf/.png
"""
...
ALL_SPLITS: List[Dict[str, str]] = [
    ...
    },
    {
        "name": "Scaffold Split",
        "tag": "scaffold",
        "train_file": "data/downstream/cycpeptmpdb_multi_assay_scaffold_train.csv",
        "test_file": "data/downstream/cycpeptmpdb_multi_assay_scaffold_test.csv",
    },
]
...
def setup_logging(log_dir: Path) -> logging.Logger:
    ...
    log_file = log_dir / f"visualize_multi_assay_splits_{timestamp}.log"
...
def run_legend(results_dir: Path) -> None:
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    _save(fig, results_dir / f"tsne_multi_assay_legend_{timestamp}")
...
def run_single(split_tag: str, results_dir: Path, jitter: float) -> None:
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    _save(fig, results_dir / f"tsne_multi_assay_{split_tag}_{timestamp}")
...
def run_combined(results_dir: Path, jitter: float) -> None:
    ...
    for split_info in ALL_SPLITS:
        ...
        Z, split_labels = get_split_data(split_info)
        panels.append((Z, split_labels, split_info["name"]))
    ...
    fig.suptitle(
        "t-SNE of Multi-Assay Splits (Morgan Fingerprints)",
        fontsize=16,
        fontweight="bold",
        y=1.02,
    )
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    _save(fig, results_dir / f"tsne_multi_assay_splits_{timestamp}")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Visualize multi-assay train/val/test split distributions via t-SNE",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "Examples:\n"
            "  python scripts/visualize_multi_assay_splits.py                     # combined\n"
            "  python scripts/visualize_multi_assay_splits.py --split random      # single\n"
            "  python scripts/visualize_multi_assay_splits.py --split scaffold    # single\n"
            "  python scripts/visualize_multi_assay_splits.py --legend            # legend only\n"
            "  python scripts/visualize_multi_assay_splits.py --jitter 0.5        # custom jitter\n"
        ),
    )
    parser.add_argument(
        "--split",
    ...
    logger.info("=" * 60)
    if args.legend:
        logger.info("Multi-Assay Split Visualization: legend only")
    elif args.split:
        logger.info(f"Multi-Assay Split Visualization: single ({args.split})")
    else:
        logger.info("Multi-Assay Split Visualization: combined (all splits)")
    logger.info("=" * 60)
